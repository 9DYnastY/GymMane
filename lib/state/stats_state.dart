...
mixin StatsState on FitCore, ToolsState, LibraryState, TimelineState {
  ...
  void setLoggedReps(LoggedExercise e, int i, int reps) {
    ...
    e.sets[i] = LoggedSet(reps.clamp(0, 999), e.sets[i].weight, kind: e.sets[i].kind);
    persistNow();
    _refreshWidgets();
    notifyListeners();
  }
  ...
  void setLoggedWeight(LoggedExercise e, int i, double kg) {
    if (i < 0 || i >= e.sets.length) return;
    e.sets[i] = LoggedSet(e.sets[i].reps, _round3(kg.clamp(0, 1000)), kind: e.sets[i].kind);
    persistNow();
    _refreshWidgets();
    notifyListeners();
  }
  ...
  void updateLoggedSet(LoggedSession s, LoggedExercise e, int setIdx, {int? reps, double? weight}) {
    ...
    if (reps != null) st.reps = reps.clamp(0, 999);
    if (weight != null) st.weight = _round3(weight.clamp(0, 1000));
    persistNow();
    _refreshWidgets();
    notifyListeners();
  }

  void addLoggedSet(LoggedExercise e, {int? reps, double? weight}) {
    final last = e.sets.isNotEmpty ? e.sets.last : null;
    final r = reps ?? last?.reps ?? 10;
    final w = weight ?? last?.weight ?? (isRepsOnly(e.id) ? 0.0 : 20.0);
